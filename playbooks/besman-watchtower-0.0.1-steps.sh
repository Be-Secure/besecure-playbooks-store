...
cd $BESMAN_WATCHTOWER_PATH/src

if [[ "$BESMAN_REPO_TYPE" == "github" ]]; then
    SCAN_CMD="python watchtower.py --repo_type=github --repo_url=$BESMAN_REPO_URL --branch_name=$BESMAN_BRANCH_NAME --depth=$BESMAN_DEPTH_VAL"
elif [[ "$BESMAN_REPO_TYPE" == "huggingface" ]]; then
    SCAN_CMD="python watchtower.py --repo_type=huggingface --repo_url=$BESMAN_REPO_URL"
else
