...
function __besman_init() {
    ...
    local steps_file_name="besman-SBOM-$ASSESSMENT_TOOL_NAME-0.0.1-steps.sh"
    ...
    if [[ $flag == true ]]; then
        ...
        export DETAILED_REPORT_PATH="$SBOM_PATH/$BESMAN_ARTIFACT_NAME-$BESMAN_ARTIFACT_VERSION-cdx-sbom-report.json"
        mkdir -p "$SBOM_PATH"
