...
function __besman_init() {
    ...
    export ASSESSMENT_TOOL_NAME="CodeQL"
    export ASSESSMENT_TOOL_TYPE="codeql"
    export ASSESSMENT_TOOL_VERSION="v2.16.5"
    ...
    if [[ $flag == true ]]; then
        ...
        export CODEQL_PATH="$BESMAN_ASSESSMENT_DATASTORE_DIR/$BESMAN_ARTIFACT_NAME/$BESMAN_ARTIFACT_VERSION/sast"
        export DETAILED_REPORT_PATH="$CODEQL_PATH/$BESMAN_ARTIFACT_NAME-$BESMAN_ARTIFACT_VERSION-codeql-report.json"
        mkdir -p "$CODEQL_PATH"
...
function __besman_prepare() {
    ...
    mv "$CODEQL_PATH"/bom-*.json "$DETAILED_REPORT_PATH"
...
function __besman_cleanup() {
    local var_array=("ASSESSMENT_TOOL_NAME" "ASSESSMENT_TOOL_TYPE" "ASSESSMENT_TOOL_PLAYBOOK" "ASSESSMENT_TOOL_VERSION" "OSAR_PATH" "CODEQL_PATH" "DETAILED_REPORT_PATH")
